...
7. **Patient identity continuity:** guest bookings create internal patient profiles keyed on verified phone; account registration claims existing profile by phone match + email verification. Merge policy is a domain rule in `identity`.
...
- Events: `identity.user_registered.v1`, `identity.role_assigned.v1`, `identity.patient_profile_created.v1`, `identity.profile_claimed.v1`.
...
- WhatsApp Cloud API adapter for provider password-recovery OTP.
