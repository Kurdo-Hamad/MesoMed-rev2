...
- **ADR filename index** (`docs/adr/` is authoritative): `0001` locked stack & Phase 0 foundation · `0002` pre-Phase-1 remediation of MM-QA-001 findings · `0003` Phase 1 kernel · `0004` Phase 2 identity · `0005` Phase 3 directory/search · `0006` Phase 4 scheduling/booking · `0007` Phase 5 clinical · `0008` Phase 6 billing/payments · `0009` Phase 6b billing revenue model · `0010` clinical extension (prescriptions & patient medical profile) · `0011` Phase 7 communication + AI.
...
- **Phase 6b (billing revenue model)** inserted between Phase 6 and Phase 7, extending the billing module: per-provider subscription-model selection (flat_monthly | commission), category × model × rate-kind rate config, the unified `billing_charges` ledger (integer minor units, DB-enforced immutability, snapshotted rates), trial windows (global default + per-provider override), and the dormant, config-gated patient cancellation/no-show collection path. Governing principle: model the full revenue shape, activate only the launch slice — deferred behavior is wired code behind config flags, never missing code. `stripe` joins fib/zaincash as a routable, fail-closed, interface-ready gateway id (§8 unchanged for live integrations). ADR-0008's non-account visibility exemption reclassified as transitional (permanent policy: every publicly visible provider holds a subscription). Recorded in ADR-0009.
- **Clinical extension (prescriptions & patient medical profile)** inserted between Phase 6b and Phase 7, extending the clinical module as a dated amendment to Phase 5: append-only `prescriptions` (amendment = new revision + atomic supersede flip, DB guard triggers), **§3.6 amended — `prescriptions` joins the clinical RLS tier** (deny-all + SECURITY DEFINER channel + audit); `patient_medical_profile` under the locked option-A model (free patient upsert, no revision history, no RLS/audit — patient-owned safety data with no falsification incentive); `patient_reported_medications` structurally distinct from clinical prescriptions in every payload; continuity-of-care history access (treating relationship = ≥1 appointment in booked/confirmed/checked_in/in_progress/completed, composed from scheduling/booking published queries). Prescription events (`clinical.prescription_{issued,amended,discontinued}.v1`, ids only) ship with no subscriber — communication subscribes in Phase 7 (MM-DEC §6); patient visit-note visibility remains unscheduled. Recorded in ADR-0010.
- **Phase 7 (communication + AI)**: event-driven notification dispatch (communication module) subscribing to booking/billing/prescription events strictly id-only, re-reading PII via published cross-module queries at plan time (never copied from event payloads — mirrors ADR-0010's PII discipline); trilingual (en/ar/ckb) templates with locale resolved from `user_channel_preferences.locale` (default ckb) before any name/copy is picked (`buildParams`-deferred-locale design, avoiding wrong-language name/template pairings); a poll-based sender with inline WhatsApp→SMS fallback (mirrors MM-DEC rev02 §8's OTP fallback order, same row updated rather than re-planned); a next-day reminder cron over a new `(status, starts_at)` booking index. AI module ports the Phase-6-deferred symptom-triage pipeline behind `AiGateway`: deterministic red-flag pre-screen unconditional before any model call, DB-whitelist intersection, deterministic keyword fallback, per-caller and global rate limits; Anthropic Claude Haiku is the concrete default model (deviation from an unpinned "Anthropic default" — cost/latency tradeoff for a whitelisted-slug-only hint, `AI_TRIAGE_MODEL` overridable). Real vendor adapters (Meta WhatsApp, Twilio SMS, Expo Push, Resend, Anthropic) join their Phase-2-vintage mocks, selected per-channel by credential presence at the composition root; a boot-time guardrail refuses `NODE_ENV=production` if any channel still resolves to its mock, naming which one. `notification_log.destination`/`.paramsJson` are the sanctioned PII storage, crypto-shred scope, 12–24 month retention (no retention job built yet — scope recorded for a future phase). Open items carried forward: MM-DEC rev03 (pending), F-07 identity-event PII posture (pending), F-09 provenance labeling (pending), locale-source placement (currently `user_channel_preferences.locale`; may move to the core identity profile in a future phase). Recorded in ADR-0011.
- **2026-07-18 — §6 log reconciled through ADR-0049** (MM-QA-004 F-17; the log had stalled at ADR-0011). Extended ADR filename index: `0012` Phase 8 caching seam · `0013` mobile API compatibility policy (frozen surface pins) · `0014` soft-delete/deactivation semantics · `0015` organization/tenant reserved design · `0016` Phase 8 web app · `0017` OTel test collector port · `0018` Phase 9a mobile toolchain · `0019` Phase 9a patient mobile app · `0020` Phase 9b provider mobile queue · `0021` Phase 9c local-gate exception · `0022` embedded-PG port race · `0023` dev-machine migration · `0024` Phase 9c delay/late-patient close-out · `0025` Phase 10 supply-chain scanning · `0026` Phase 10 observability · `0027` Phase 10 DB role + backup drill · `0028` Phase 10 retention/erasure · `0029` Supabase migration no-op · `0030` Phase 10 load test + index audit · `0031` Phase 10 launch checklist close-out (now carrying the MM-QA-004 disposition, owner-override, HG-2 and password-recovery amendments) · `0032`–`0049` the MM-QA-004 remediation slices (each ADR names its finding; see the ADR-0031 close-out amendment for the finding → PR map).
- **2026-07-18 — unlogged edit `084214e` (2026-07-10) recorded** (MM-QA-004 F-17): that commit rewrote this plan's locked stack table and §3 convention #7 to reconcile with MM-DEC rev02 (patient phone+password, OTP-verified signup, WhatsApp/SMS recovery role) without a §6 entry. The reconciliation was owner-directed (MM-DEC rev02 adoption); this entry restores the audit trail.
- **2026-07-18 — Phase 3 lineage note** (MM-QA-004 F-15): Phase 3's GitHub PR merge commit (`110dd53`) is not in `main`'s first-parent history — Phase 3 landed via `ba48bd9`; audit-trail correction to the MM-QA-002 F-02 lineage only, no content impact.
- **2026-07-19 — multi-country catalog expansion** (standalone slice; recorded in ADR-0055): the launch catalog goes multi-country — nine new cities, eight new categories, five additional live countries, country-scoped list surfaces, a `search_documents.country_iso` column and a web country switcher. Two statements in this plan are amended by it. (a) **§8's "medical marketplace category" deferral is overridden, narrowly, by owner ruling:** the category may exist as a taxonomy row, a homepage tile and a trilingual Coming Soon landing (`robots: noindex`, zero providers seeded) — no marketplace domain, tables, events, listings or payments are built, so §8 otherwise stands. This is ADR-0009's governing principle applied to the catalog: deferred behavior is wired code behind config flags, never missing code. Deferred-visible status is a new config row (`directory.category_gating`, category slug → `active | coming_soon`) mirroring ADR-0005's country gating but failing **open** (unlisted category = active) — the categories table is the authority on what exists, so a config blip must not blank the catalog; non-`NOT_FOUND` failures still propagate. (b) **§5 Phase 8's "directory + detail pages (7 categories + home-nursing)" is superseded:** the homepage tile set is no longer fixed in web code but resolved per country from a second config row (`directory.category_display`, ISO2 → ordered tile ids) that also fails open (unlisted country = the full active category list, which is how Iraq is served); `doctors` is a reserved tile id, not a category row. Registration is contract + column only (`completeProviderSignup.countryCode`, `provider_profiles.country_code` NOT NULL DEFAULT `'IQ'`); the step-2 client form is deliberately not built and is carried as the named pre-launch follow-up slice `provider-registration-step2-client`. New `ar`/`ckb` strings are machine-drafted and remain an open human gate.
- **2026-07-13 — §3 convention #15 added** (branch → PR → merge; no direct pushes to `main`; `.githooks/pre-push` + `core.hooksPath` enforcement). Origin: MM-QA-002 F-02 remediation (the false/inert branch-protection claim). Restated verbatim as CLAUDE.md convention #15; no standalone ADR file — process convention, not an architecture decision.
- **2026-07-20 — §6 log extended through ADR-0057** (MM-QA-005 F-05; the `0032`–`0049` phrasing above left five later remediation slices and the vocabulary-drift/disposition ADRs unindexed). Extended ADR filename index: `0050` MM-QA-004 F-12 clinical list bounds · `0051` MM-QA-004 F-13 ar/ckb search normalization · `0052` MM-QA-004 doc bundle (F-15, F-17, F-26, F-27, F-28) · `0053` MM-QA-004 i18n trio (F-22, F-23, F-24) · `0054` MM-QA-004 F-25 search seq-scan revisit trigger · `0055` multi-country catalog expansion (already logged above) · `0056` provider-type vocabulary drift (single-sourced CHECK, billing vocabulary, seed schema preflight) · `0057` MM-QA-005 full-system audit disposition (this remediation).
