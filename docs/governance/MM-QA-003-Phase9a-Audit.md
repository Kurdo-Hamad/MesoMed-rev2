...
4. Consequence stronger than ADR-0019 states: because only turbo's
   *task-level* `exited (1)` survived, it cannot even be confirmed that a
...
§3.7): not re-run-to-green (three *forced* serialized runs + a standalone api
...
(`corepack pnpm` from the nvm node bin) covers only the *top-level*
...
(`app/auth/sign-up.tsx` — the *library* claim path is integration-tested,
...
nothing asserts that keys the mobile app *consumes* exist at all: a
...
item, so the *capability* is tracked — the flow-list divergence is a
