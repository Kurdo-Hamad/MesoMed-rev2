...
| `user` / auth tables (Better Auth)             | email, name, sessions                                                                                                                                                                                                                                                                                                                                                                | life of account                                   | self-service account deletion (`identity.deleteAccount`, MM-QA-004 F-02 / ADR-0033): deletes the user + sessions, cascades auth/role/device/provider rows, anonymizes the patient profile, prunes `notification_log` |
| `domain_events`                                | event payloads — **id-only as of migration 0010 — redaction verified in test against legacy-shaped rows; production verification lands with deploy (D10)**: identity events emit id-only v2 contracts, stored identity v1 payloads redacted by migration 0010 (`phone`/`email`/`normalizedPhone` removed; MM-QA-004 F-04, closing MM-QA-002 F-07); booking snapshots carry ids/times | operational history                               | keep (pseudonymous)                                                                                                                                                                                                  |
