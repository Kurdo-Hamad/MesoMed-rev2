...
describe("createMetaWhatsAppAdapter", () => {
  ...
  it("wraps a non-ok response as NotifySendError without leaking the token", async () => {
  ...
  it("renders the OTP message from the locale catalog", async () => {
    ...
    await adapter.otp.send({ to: "+9647701234567", code: "482913", locale: "en" });
  ...
  it("wraps OTP delivery failure as OtpSendError", async () => {
    ...
    await expect(
      adapter.otp.send({ to: "+9647701234567", code: "482913", locale: "en" }),
