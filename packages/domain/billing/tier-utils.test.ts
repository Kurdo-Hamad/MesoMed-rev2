...
describe('computeNewExpiry', () => {
  it('extends from a future expiry', () => {
    ...
    expect(computeNewExpiry(current, 1, NOW).toISOString()).toBe(
  ...
  it('starts from now when expiry is past or absent', () => {
    ...
    expect(computeNewExpiry(null, 1, NOW).toISOString()).toBe(
      '2026-08-07T12:00:00.000Z'
    );
  });

  it('adds one month per period', () => {
    expect(computeNewExpiry(null, 3, NOW).toISOString()).toBe(
  ...
  it('rolls over year boundaries', () => {
    ...
    expect(computeNewExpiry(dec, 2, NOW).toISOString()).toBe(
